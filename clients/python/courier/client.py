...
class Client:
    ...
    def create_topic(self, name, config):
        url = urljoin(
            self.base_url, "{0}/{1}".format(Client.topics_path, name))
    ...
    def list_topics(self):
        url = urljoin(
            self.base_url, "{0}/".format(Client.topics_path))
        resp = requests.get(url)
        resp.raise_for_status()
        return resp.json()
    ...
    def publish(self, topic, data):
        ...
        url = urljoin(
            self.base_url, "{0}/{1}/publish".format(Client.topics_path, topic))
    ...
    def get_topic_subscriptions(self, topic):
        url = urljoin(
            self.base_url, "{0}/{1}/subscriptions".format(Client.topics_path, topic))
        resp = requests.get(url)
        resp.raise_for_status()
        return resp.json()
    ...
    def create_subscription(self, name, config):
        url = urljoin(
            self.base_url, "{0}/{1}".format(Client.subscriptions_path, name))
        resp = requests.put(url, json=config)
        resp.raise_for_status()
        return resp.json()
    ...
    def list_subscriptions(self):
        url = urljoin(
            self.base_url, "{0}/".format(Client.subscriptions_path))
        resp = requests.get(url)
        resp.raise_for_status()
        return resp.json()
    ...
    def pull(self, subscription, max_messages=1):
        url = urljoin(
            self.base_url, "{0}/{1}/pull".format(Client.subscriptions_path, subscription))
    ...
    def ack(self, subscription, uuids):
        ...
        if isinstance(uuids, basestring):
            ...
            message_ids = uuids
        url = urljoin(
            self.base_url, "{0}/{1}/ack".format(Client.subscriptions_path, subscription))
